...
----------------------------------------
- Fixed problem with certain CSS styles in Vuetify spilling over into classic SharePoint.
- Fixed missing localization label for the Close button in the Strengthen Profile block.
- Fixed issues with the Strenghten Profile UI on a mobile device.
...
- Fixed some general localization problems with new modern blocks.

Omnia.Intranet (1.0.13697)
----------------------------------------
- If central naming policies is applied to Office 365 Groups, the create site wizard will raise a warning if the name of the new site does not align with the policies. (In a coming release, the create site UI will help the user to be compliant with the policies).
- Fixed issue with german localization in Aggregated Calendar.
- Added a possibility to for web authors to see which status the page is in (draft, published, waiting for approval). It the page is not published an info icon will be shown next to the Quick Edit button on the page.)
- Made sure a page is not checked out when clicking on the quick edit button and the page is in waiting for approval status.
...
Omnia.DocumentManagement (1.0.13677)
----------------------------------------
- Fixed problem with the metadata replace feature in DOCX making the page no fields corrupt.
